...
// NewStructTypeStrategy 创建新的结构体类型策略
func NewStructTypeStrategy() *StructTypeStrategy {
	return &StructTypeStrategy{
		BaseStrategy: NewBaseStrategy("StructType", 10),
	}
}

// CanConvert 检查是否可以转换结构体类型
func (sts *StructTypeStrategy) CanConvert(t *core.TypeInfo) bool {
	if t == nil || t.Type == nil {
		return false
	}
...
// Convert 转换结构体类型
func (sts *StructTypeStrategy) Convert(ctx *ConversionContext) (string, error) {
	...
	// 生成包别名
	alias := sts.generatePackageAlias(pkgPath)
...
func (sts *StructTypeStrategy) generatePackageAlias(pkgPath string) string {
	// 处理标准库包
	switch pkgPath {
	case "net/http":
		return "httpsrc"
	...
	default:
		// 对于第三方包，使用包名的缩写
		return sts.getShortPackageName(pkgPath)
	}
}

// getShortPackageName 获取包的短名称
func (sts *StructTypeStrategy) getShortPackageName(pkgPath string) string {
	// 提取包名
	parts := []string{}
