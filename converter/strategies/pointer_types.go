...
// NewPointerTypeStrategy 创建新的指针类型策略
func NewPointerTypeStrategy() *PointerTypeStrategy {
	return &PointerTypeStrategy{
		BaseStrategy: NewBaseStrategy("PointerType", 40),
	}
}

// CanConvert 检查是否可以转换指针类型
func (pts *PointerTypeStrategy) CanConvert(t *core.TypeInfo) bool {
	if t == nil || t.Type == nil {
		return false
	}
...
// Convert 转换指针类型
func (pts *PointerTypeStrategy) Convert(ctx *ConversionContext) (string, error) {
	...
	elemTypeInfo := &core.TypeInfo{
		...
		PackageName: pts.getPackageName(elemType.PkgPath()),
	...
	// 如果无法获取策略，使用默认转换
	elemTypeStr := pts.getDefaultTypeString(elemType)
	return "*" + elemTypeStr, nil
}

// getPackageName 获取包名
func (pts *PointerTypeStrategy) getPackageName(pkgPath string) string {
	if pkgPath == "" {
		return ""
	}
...
func (pts *PointerTypeStrategy) getDefaultTypeString(t reflect.Type) string {
	if t == nil {
		return "interface{}"
	}
	...
	case reflect.Interface:
		return "interface{}"
	case reflect.Ptr:
		return "*" + pts.getDefaultTypeString(t.Elem())
	case reflect.Struct:
		if t.PkgPath() != "" {
